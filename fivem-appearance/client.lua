...
Citizen.CreateThread(function()
	while true do
		...
		if (isInClothingShop and not hasAlreadyEnteredMarker) or (isInClothingShop and LastZone ~= currentZone) then
			...
			TriggerEvent('cd_drawtextui:ShowUI', 'show', "Premi [E] Per Cambiare Vestiti")
		end
		...
		if (isInBarberShop and not hasAlreadyEnteredMarker) or (isInBarberShop and LastZone ~= currentZone) then
			...
			TriggerEvent('cd_drawtextui:ShowUI', 'show', "Premi [E] Per Cambiare Capelli")
		end
...
RegisterNetEvent('fivem-appearance:clothingShop', function()
    TriggerEvent('zf_context:sendMenu', {
        {
            id = 1,
            header = "Cambia Vestiti",
            txt = "",
			params = {
...
RegisterNetEvent("boston:reskin")
AddEventHandler("boston:reskin", function()
	local config = {
		ped = true,
		headBlend = true,
		faceFeatures = true,
